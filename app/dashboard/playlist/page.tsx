...
export default function PlaylistPage() {
  ...
  if (error) {
    const isNoCouple = error.includes('Couple ID not found') || error.includes('paired with a partner') || !coupleId;
    return (
      <div className="p-6 bg-white rounded-xl border border-gray-200 text-center max-w-md mx-auto">
        ...
        <p className="text-gray-700 mb-4">
          {isNoCouple
            ? 'Invitez votre partenaire depuis l\'accueil pour accéder à la playlist.'
            : error}
        </p>
        {isNoCouple && (
